// /src/components/InfoPanel.jsx (最終版 - 新增一鍵清除)
...
// 為了方便您直接複製貼上，這裡提供完整的 InfoPanel.jsx 內容
const FullInfoPanel = () => {
    const {
        storageSpaces, selectedSpace, setStorageSpace,
        getCalculations, itemsInScene, removeItemFromScene, setCustomSpace,
    ...
    return (
        <div className="absolute top-4 right-4 bg-gray-800/70 backdrop-blur-sm text-white p-4 rounded-xl shadow-lg w-80 z-10 max-h-[calc(100vh-2rem)] overflow-y-auto">
            ...
            <div className="mb-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                    {Object.keys(storageSpaces).filter(s => s !== 'Custom').map((size) => (
                        <button key={size} onClick={() => setStorageSpace(size)} className={`p-3 rounded-lg transition-all duration-200 shadow-md text-sm font-bold ${selectedSpace === size ? 'bg-green-500 text-white ring-2 ring-white/50' : 'bg-gray-700 text-gray-200 hover:bg-gray-600'}`}>{size}</button>
                    ))}
                </div>
            </div>
            ...
            <div>
                ...
                <div className="max-h-32 overflow-y-auto space-y-1 p-2 bg-gray-900/50 rounded-lg border border-gray-700">
                    {itemsInScene.length === 0 ? (<p className="text-sm text-gray-400 text-center py-4">請從左側新增物品</p>) : (
                        itemsInScene.map(item => (
                            <div key={item.instanceId} className="flex justify-between items-center bg-gray-700 p-2 rounded-md shadow-sm text-sm">
                                ...
                                <button onClick={() => removeItemFromScene(item.instanceId)} className="text-red-400 hover:text-red-300 font-bold px-2 flex-shrink-0">×</button>
                            </div>
                        ))
                    )}
                </div>
...
export { FullInfoPanel as default };